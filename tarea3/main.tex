...
A nivel visual, el comportamiento asíncrono y la ocupación de los recursos se pueden confirmar mediante la inspección del \textit{timeline} del profiler. 

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{nsys_host_gpu.png}
    \caption{Panorama general de ocupación Host-GPU: se observa la coordinación entre el despacho de tareas en los núcleos de la CPU (Host, sección superior) y la ejecución en la GPU (Device, sección inferior). El desfase temporal confirma que el Host despacha operaciones mientras la GPU procesa lotes anteriores.}
    \label{fig:nsys_host_gpu}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{nsys_overlap.png}
    \caption{Detalle a nivel de dispositivo (GPU): ejecución concurrente de transferencias de memoria (\textit{MemCpy}) y cómputo (\textit{Kernels}) a través de múltiples CUDA Streams, lo que permite ocultar parcialmente las latencias.}
    \label{fig:nsys_overlap}
\end{figure}
% ============================================================================
